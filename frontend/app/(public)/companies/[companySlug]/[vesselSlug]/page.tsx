...
export default function VesselDetailPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!foundComp || !foundVessel) {
        setError("Vessel or Company records not found.");
        return;
      }
      ...
      console.error("Failed to query vessel allocations details", err);
  ...
  useEffect(() => {
    if (companySlug && vesselSlug) {
      loadData();
    }
  }, [companySlug, vesselSlug]);

  const handleUpdateVessel = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!vessel || !company) return;
    setFormError(null);
    setFormSuccess(null);
    ...
    try {
      ...
      setTimeout(() => {
        ...
        router.push(`/companies/${companySlug}/${newVesselSlug}`);
      }, 1500);
    } catch (err: any) {
    ...
    <>
      <PublicLayoutHeader>
        <div className="flex flex-col gap-2">
          <Link href={`/companies/${companySlug}`} className="text-xs text-primary hover:underline font-mono">
          ...
          <span className="font-mono text-xs text-muted-soft">IMO: {vessel.imo} | flag: {vessel.flag}</span>
        </div>
      </PublicLayoutHeader>
