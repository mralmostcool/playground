...
export default function HomePage() {
  return (
    <div className="w-full max-w-6xl mx-auto px-6 py-4 flex flex-col items-center justify-center overflow-hidden">
      ...
      {/* Portal Gateway styled Options Grid */}
      <section className="w-full max-w-5xl flex-shrink-0">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            <div>
              {/* Icon Container */}
              <div className="w-10 h-10 rounded-lg bg-primary/10 text-primary flex items-center justify-center mb-5 transition-colors duration-300 group-hover:bg-primary group-hover:text-on-primary">
              ...
              <h2 className="text-lg font-serif text-ink mb-2 group-hover:text-primary transition-colors duration-300">
                Seafarer Profile Portal
              </h2>
              <p className="text-xs text-body-text mb-4 leading-relaxed">
                Access active seafarer registries, look up compliance profiles, check active ranks, and verify INDOS certifications.
              </p>

              <ul className="text-xxs text-muted space-y-1.5 mb-6 border-t border-hairline-soft pt-4 hidden sm:block">
          ...
          {/* Card 2: Pre-Sea Courses & Institutes */}
          <Link 
            href="/seafarer" 
          ...
          >
            <div>
              {/* Icon Container */}
              <div className="w-10 h-10 rounded-lg bg-accent-teal/10 text-accent-teal flex items-center justify-center mb-5 transition-colors duration-300 group-hover:bg-accent-teal group-hover:text-on-primary">
                <svg className="w-5 h-5 fill-none stroke-current" viewBox="0 0 24 24" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M22 10v6M2 10l10-5 10 5-10 5zM6 12v5c0 2 2 3 6 3s6-1 6-3v-5" />
                </svg>
              </div>

              <h2 className="text-lg font-serif text-ink mb-2 group-hover:text-accent-teal transition-colors duration-300">
          ...
          {/* Card 3: Berth Schedules */}
          <Link 
            href="/seafarer" 
            className="group relative flex flex-col justify-between p-6 md:p-8 bg-surface-card hover:bg-canvas border border-hairline hover:border-accent-amber rounded-xl transition-all duration-300 text-left shadow-sm hover:shadow-md hover:-translate-y-0.5"
          >
            <div>
              {/* Icon Container */}
              <div className="w-10 h-10 rounded-lg bg-accent-amber/10 text-accent-amber flex items-center justify-center mb-5 transition-colors duration-300 group-hover:bg-accent-amber group-hover:text-on-primary">
                <svg className="w-5 h-5 fill-none stroke-current" viewBox="0 0 24 24" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
                </svg>
              </div>

              <h2 className="text-lg font-serif text-ink mb-2 group-hover:text-accent-amber transition-colors duration-300">
                Berths & Vessels Info
              </h2>
              <p className="text-xs text-body-text mb-4 leading-relaxed">
                Verify active berth statuses, vessel registrations, shipping company profiles, and public allocation timelines.
              </p>

              <ul className="text-xxs text-muted space-y-1.5 mb-6 border-t border-hairline-soft pt-4 hidden sm:block">
                <li className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-accent-amber" />
                  <span>Public Vessel Registers</span>
                </li>
                <li className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-accent-amber" />
                  <span>Berthing Slot Timelines</span>
                </li>
                <li className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-accent-amber" />
                  <span>Active Shipping Companies</span>
                </li>
              </ul>
            </div>

            <div className="flex items-center gap-2 text-xs md:text-sm font-semibold text-accent-amber mt-auto">
              <span>Explore Berth Schedules</span>
              <svg className="w-3.5 h-3.5 transform transition-transform duration-300 group-hover:translate-x-1.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3" />
              </svg>
            </div>
